...
/**
 * The RecipeStation class is a station representing the place in the kitchen where you combine
 * ingredients to create food. The food can then be served to the customer via the station.
 */
public class RecipeStation extends Station {
   ...
   * @param textureManager The controller from which we can get information on what texture each
   *     ingredient should have
   * @param customerManager The controller from which we can get information on what food needs to
   *     be served
   */
  ...
  public List<ActionType> getActionTypes() {
    ...
    if (nearbyChef != null) {
      if (!nearbyChef.getStack().isEmpty()) {
        ...
        if (checkItem.getIsChopped()
            || checkItem.getIsCooked()
            || Objects.equals(checkItem.getType(), "bun")
            || Objects.equals(checkItem.getType(), "cheese")) {
