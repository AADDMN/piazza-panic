...
public class ChefManager implements Disposable {
  ...
  /**
   * @param chefScale the amount to scale the texture by so that each chef is an accurate size.
   * @param collisionLayer the tile map layer which the chefs can collide with.
   * @param overlay the user interface overlay to display information about the current chef and
   *     time, and to provide more controls.
   */
    ...
    for (int i = 0; i < chefSprites.length; i++) {
      ...
      Chef chef =
          new Chef(
              ...
              this);
      chef.setBounds(
          UIOverlay.chefXsave.get(i),
          UIOverlay.chefYsave.get(i),
